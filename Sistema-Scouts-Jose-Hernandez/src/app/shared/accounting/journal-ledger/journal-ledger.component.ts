...
export class JournalLedgerComponent implements OnInit, OnDestroy {
  ...
  exportToPDF(): void {
    ...
    const filename = `libro-diario-${startDate}-${endDate}`;
    const title = 'Libro Diario - Sistema Scout José Hernández';
  ...
  private prepareExportData(): any[] {
    return this.entries.map(entry => ({
      ...
      originType: this.ORIGIN_TYPE_LABELS[entry.originType],
      status: this.ENTRY_STATUS_LABELS[entry.status],
